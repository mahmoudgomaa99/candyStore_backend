...
const createUser = async (req, res, next) => {
  ...
  if (!phone_number || !user_type || !name || !password) {
    return next(
  ...
  if (req.file) {
    ...
    await bucket
      ...
      .then((res) => {
        currentUser.store_img_url =
          "https://storage.googleapis.com/food-560b9.appspot.com/" +
          req.file.filename;
  ...
  res.setHeader("uid", createdUser.id);
...
const login = async (req, res, next) => {
  ...
  res.setHeader("token", token);
  res.setHeader("uid", existingUser.id);
...
const updateUser = async (req, res, next) => {
  ...
  let currentUser;
  try {
    currentUser = await User.findById(uid).select(["-password", "-token"]);
  } catch (error) {
    return next(
      new HttpError("failed updating the user, please try again", 500)
    );
...
const sendOtp = async (req, res, next) => {
  ...
  try {
    await client.messages.create({
      to: phone_number,
      from: "+19062890141",
      body: `Your Snacks Store App OTP is ${otp}`,
    });
  } catch (error) {
    console.log(error);
...
const fetchUser = async (req, res, next) => {
  const { uid, authorization } = req.headers;
  if (!uid) {
    return next(new HttpError("invalid inputs, please check your data", 422));
  }
  let user;
...
const fetchUserById = async (req, res, next) => {
  ...
  let user;
  try {
    user = await User.findById(id).select(["-password", "token"]);
  } catch (error) {
    return next(new HttpError("couldn't get the user, please try again", 500));
  }
  if (!user) {
